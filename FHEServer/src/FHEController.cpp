...
void FHEController::load_server_context(bool verbose) {
    ...
    const string budget = read_from_file(root + "level_budget.txt");
    level_budget[0] = budget.at(0) - '0';
    level_budget[1] = budget.at(2) - '0';
    const uint32_t levelsUsedBeforeBootstrap = get_relu_depth(relu_degree) + 3;
    circuit_depth = levelsUsedBeforeBootstrap +
        FHECKKSRNS::GetBootstrapDepth(8, level_budget, SPARSE_TERNARY);
    num_slots = 1 << 14;
    if (verbose) cout << "Server context loaded without a secret key." << endl;
}
...
Ctxt FHEController::add(const Ctxt &c, const Ptxt &p) {
    return context->EvalAdd(c, p);
}

...
int main(int argc, char *argv[]) {
    ...
    if (generate_context == -1) {
        cerr << "You either have to use the argument \"generate_keys\" or \"load_keys\"!\nIf it is your first time, you could try "
                "with \"./LowMemoryFHEWeaponResNet20 generate_keys 1\"\nCheck the README.md.\nAborting. :-(" << endl;
        exit(1);
    }
...
Ctxt final_layer(const Ctxt& in) {
    ...
    if (verbose >= 0) {
        ...
        controller.print(res, 2, "Output: ");
    }

    vector<double> clear_result = controller.decrypt_tovector(res, 2);
    ...
    if (verbose >= 0) {
        ...
        if (plain) {
            string command = "../.venv/bin/python ../src/plain/script.py \"" + input_filename + "\"";
...
vector<double> read_image(const char *filename) {
    ...
    if (width != 32 || height != 32 || channels != 3) {
        cerr << "Input must be a 32x32 RGB image: " << filename << endl;
    ...
    for (int i = 0; i < width * height; ++i) {
        //Channel R
        imageVector.push_back((static_cast<double>(image_data[3 * i]) / 255.0f - 0.5) / 0.5);
    }
    for (int i = 0; i < width * height; ++i) {
        //Channel G
        imageVector.push_back((static_cast<double>(image_data[1 + 3 * i]) / 255.0f - 0.5) / 0.5);
    }
    for (int i = 0; i < width * height; ++i) {
        //Channel B
        imageVector.push_back((static_cast<double>(image_data[2 + 3 * i]) / 255.0f - 0.5) / 0.5);
    }
